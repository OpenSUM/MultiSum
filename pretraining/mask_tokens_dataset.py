...
class MaskTokensDataset(BaseWrapperDataset):
    ...
    def __init__(
        ...
        if extra_dict!=None:
            self.extra_dict=open(extra_dict).readlines()
            for i in range(len(self.extra_dict)):
                self.extra_dict[i]=int(self.extra_dict[i].replace('\n',''))
            self.extra_dict=set(self.extra_dict)
        else:
            self.extra_dict=None
        print('extra_dict=',self.extra_dict)
    ...
    def __getitem_cached__(self, seed: int, epoch: int, index: int):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            if self.extra_dict==None:
                ...
                mask_idc = []
                for i in range(sz):
                    if item[i].item() in self.extra_dict:
                        mask_idc.append(i)
                mask_idc = np.array(mask_idc).astype('int64')
                #print('mask_idc=',len(mask_idc))
                if num_mask-len(mask_idc)>0:
                    mask_idc = np.concatenate((mask_idc,np.random.choice(sz, num_mask-len(mask_idc), replace=False))).astype('int64')
                #if len(mask_idc)==0:
                #    mask_idc=np.array([])
                #mask_idc = np.random.choice(sz, num_mask, replace=False)
