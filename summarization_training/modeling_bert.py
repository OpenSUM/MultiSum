...
from utils.my_attention import trim_relative_attention
        ...
        with tf.variable_scope("embeddings", reuse=tf.AUTO_REUSE):
            ...
            if encoder_output is not None:
                with tf.variable_scope('shift'):
                    start_embedding = encoder_output[-1][:, 0, :] if config.align_layers else encoder_output[:, 0, :]
...
def my_attention_layer(from_tensor,
                       to_tensor,
                       attention_mask=None,
    ...
    context_layer = trim_relative_attention(q=query_layer,
                                            k=key_layer,
                                            v=value_layer,
            ...
            with tf.variable_scope("attention"):
                ...
                with tf.variable_scope("self"):
                    ...
                    attention_head = func(
                        ...
                        batch_size=batch_size,
                        from_seq_length=seq_length,
                        to_seq_length=memory_tensor_seq_length if is_decoding else seq_length,
...
def reshape_from_matrix(output_tensor, orig_shape_list):
    ...
    if len(orig_shape_list) == 2 or len(output_tensor.shape) == len(orig_shape_list):
        return output_tensor
